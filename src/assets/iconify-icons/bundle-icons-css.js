...
// Install the necessary dependencies for Iconify
import { cleanupSVG, importDirectory, isEmptyColor, parseColors, runSVGO } from '@iconify/tools'
import { getIcons, getIconsCSS, stringToIcon } from '@iconify/utils'
...
// Correctly load the JSON files
const sources = {
  json: [
    // Dynamically import the JSON file
    import('@iconify/json/json/tabler.json'),

    // Add any other JSON files dynamically if needed
  ],
  svg: []
};

// File to save the generated bundle
const target = join(__dirname, 'generated-icons.css');
...
(async function () {
  ...
  if (sources.json) {
    for (let i = 0; i < sources.json.length; i++) {
      ...
      // Dynamically load the JSON content
      const content = await item;

      // Process the icons from the loaded JSON
      allIcons.push(content);
    }
  }
...
// Function to organize the icons (as per your original code)
function organizeIconsList(icons) {
  const sorted = Object.create(null);
